...
* в зависимости от валюты счета, либо 47423810*******2****, либо 47423840*******2****
* Если место проведения операции не наши устройства, то работают
* в зависимости от валюты счета, либо 47423810*******1****, либо 47423840*******1****
...
* num_47423_no_rur_atm - переменная содержит номер счета для отражения операций по чужому АТМ по условию: карта "RUR" получение наличных
* num_47423_no_usd_atm - переменная содержит номер счета для отражения операций по чужому АТМ по условию: карта "USD" получение наличных
* num_47423_no_eur_atm - переменная содержит номер счета для отражения операций по чужому АТМ по условию: карта "EUR" получение наличных

* num_47423_no_rur_pos - переменная содержит номер счета для отражения операций по чужому POS по условию: карта "RUR" получение наличных
* num_47423_no_usd_pos - переменная содержит номер счета для отражения операций по чужому POS по условию: карта "USD" получение наличных
* num_47423_no_eur_pos - переменная содержит номер счета для отражения операций по чужому POS по условию: карта "EUR" получение наличных

* num_47423_beznal_rur - переменная содержит номер счета для отражения операций по безналичным операциям в рублях
* num_47423_beznal_usd - переменная содержит номер счета для отражения операций по безналичным операциям в долларах
* num_47423_beznal_eur - переменная содержит номер счета для отражения операций по безналичным операциям в евро

* num_47423_no_rur_mag - переменная содержит номер счета для отражения операций по чужому TERM по условию: карта "RUR" оплата покупок
* num_47423_no_usd_mag - переменная содержит номер счета для отражения операций по чужому TERM по условию: карта "USD" оплата покупок
* num_47423_no_eur_mag - переменная содержит номер счета для отражения операций по чужому TERM по условию: карта "EUR" оплата покупок
...
DO CASE
  CASE ALLTRIM(m.kod_oper) == '32N'  && Код списания комиссии за покупку валюты с карточного счета
    ...
    DO start_spis_komis IN Docum_47423
  ...
  CASE ALLTRIM(m.kod_oper) == '52Z'  && Код закрытия карты весь остаток с карточного счета должен списаться на транзитный счет
    ...
    DO CASE
      CASE pr_imp_70601 = .T.  && Формирование счета по кредиту по НОВОМУ алгоритму
        ...
        DO start_zakr_liz_new IN Docum_47423

      CASE pr_imp_70601 = .F.  && Формирование счета по кредиту по СТАРОМУ алгоритму

        DO start_zakr_liz_str IN Docum_47423
  ...
  CASE INLIST(ALLTRIM(m.kod_oper), '52T', '52M', '52L', '52J', '52F', '52U', '52W')
    ...
    DO CASE
      CASE pr_imp_70601 = .T.  && Формирование счета по кредиту по НОВОМУ алгоритму
        ...
        DO start_tarif_pr_rasch_new IN Docum_47423

      CASE pr_imp_70601 = .F.  && Формирование счета по кредиту по СТАРОМУ алгоритму

        DO start_tarif_pr_rasch_str IN Docum_47423
  ...
  CASE INLIST(ALLTRIM(m.kod_oper), '12A', '12B', '52R', '52O', '52S', '52K', '52P', '52N', '52D')
    ...
    DO CASE
      CASE pr_imp_70601 = .T.  && Формирование счета по кредиту по НОВОМУ алгоритму
        ...
        DO start_70107_52_new IN Docum_47423

      CASE pr_imp_70601 = .F.  && Формирование счета по кредиту по СТАРОМУ алгоритму

        DO start_70107_52_str IN Docum_47423
    ...
    DO CASE
      CASE flag_474_303 = .F.  && Формируется проводка 40817 - 30301
        ...
        IF INLIST(ALLTRIM(m.kod_oper), '314', '31G')
          DO start_30302_obr IN Docum_47423
        ELSE
          DO start_30301 IN Docum_47423
        ENDIF

      CASE flag_474_303 = .T.  && Формируется проводка 40817 - 47423
          ...
          CASE EMPTY(ALLTRIM(m.name_term)) = .F.  && Наименование устройства через которое проведена операция НЕ ПУСТОЕ
            ...
            DO CASE
              CASE poisk_loc_term = .T.  && поиск в справочнике наших устройств прошел УСПЕШНО
                ...
                DO start_47423_loc_term IN Docum_47423
              ...
              CASE poisk_loc_term = .F.  && поиск в справочнике наших устройств прошел НЕ УСПЕШНО
                ...
                DO CASE
                  CASE poisk_exp_term = .T.  && поиск в справочнике чужих устройств прошел УСПЕШНО
                    ...
                    DO start_47423_exp_term_yes IN Docum_47423

                  CASE poisk_exp_term = .F.  && поиск в справочнике чужих устройств прошел НЕ УСПЕШНО

                    DO start_47423_exp_term_no IN Docum_47423
...
* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ *
  ...
  CASE (INLIST(LEFT(ALLTRIM(m.kod_oper), 2), '20', '22') OR INLIST(ALLTRIM(m.kod_oper), '12A', '12B', '12C', '12S', '12T', '12V', '129', '312', '31M', '31N')) ;
    ...
    DO CASE
      ...
      CASE flag_474_303 = .T.  && Формируется проводка 42301 - 47423
...
* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ *
  ...
  CASE INLIST(ALLTRIM(m.kod_oper), '113', '115', '51P')
    ...
    DO start_beznal IN Docum_47423
...
* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ *
  ...
  CASE ALLTRIM(m.kod_oper) == '516'
    ...
    DO start_70203 IN Docum_47423

* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ *

  CASE ALLTRIM(m.kod_oper) == '110'

    DO start_20202 IN Docum_47423
...
* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ *
  ...
  CASE pr_rasch_nds = .F. AND ALLTRIM(m.kod_oper) == '991'
    ...
    DO start_60322_70107 IN Docum_47423

* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ *

  CASE pr_rasch_nds = .F. AND ALLTRIM(m.kod_oper) == '992'

    DO start_60322_60309 IN Docum_47423

* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ *

  CASE pr_rasch_nds = .T. AND ALLTRIM(m.kod_oper) == '992'

    DO start_42301_60309 IN Docum_47423
...
IF ALLTRIM(num_branch) <> '01'
  DO select_kod_oper IN Docum_47423
ENDIF
* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ *
...
DO CASE
  ...
  CASE NOT INLIST(ALLTRIM(m.branch), '10')
    ...
    DO CASE
      CASE ALLTRIM(m.val_card) == 'RUR'
        m.nls_cred = ALLTRIM(num_47423_beznal_rur)

      CASE ALLTRIM(m.val_card) == 'USD'
        m.nls_cred = ALLTRIM(num_47423_beznal_usd)

      CASE ALLTRIM(m.val_card) == 'EUR'
        m.nls_cred = ALLTRIM(num_47423_beznal_eur)
...
DO CASE
  CASE ALLTRIM(m.val_card) == 'RUR'
    m.nls_deb = ALLTRIM(num_47423_beznal_rur)

  CASE ALLTRIM(m.val_card) == 'USD'
    m.nls_deb = ALLTRIM(num_47423_beznal_usd)

  CASE ALLTRIM(m.val_card) == 'EUR'
    m.nls_deb = ALLTRIM(num_47423_beznal_eur)
...
**************************************************** PROCEDURE start_47423_exp_term_yes *****************************************************************
  ...
  CASE INLIST(ALLTRIM(exp_term.term), 'A') AND INLIST(ALLTRIM(m.kod_oper), '207', '209', '225', '227', '324', '314', '31G', '32G')
    ...
    DO CASE
      CASE INLIST(ALLTRIM(m.kod_oper), '207', '209', '324', '32G')
        ...
        DO CASE
          CASE ALLTRIM(m.val_card) == 'RUR'
            m.nls_cred = ALLTRIM(num_47423_no_rur_atm)

          CASE ALLTRIM(m.val_card) == 'USD'
            m.nls_cred = ALLTRIM(num_47423_no_usd_atm)

          CASE ALLTRIM(m.val_card) == 'EUR'
            m.nls_cred = ALLTRIM(num_47423_no_eur_atm)
      ...
      CASE INLIST(ALLTRIM(m.kod_oper), '227', '229', '314', '31G')
        ...
        DO CASE
          CASE ALLTRIM(m.val_card) == 'RUR'
            m.nls_deb = ALLTRIM(num_47423_no_rur_atm)

          CASE ALLTRIM(m.val_card) == 'USD'
            m.nls_deb = ALLTRIM(num_47423_no_usd_atm)

          CASE ALLTRIM(m.val_card) == 'EUR'
            m.nls_deb = ALLTRIM(num_47423_no_eur_atm)
  ...
  CASE INLIST(ALLTRIM(exp_term.term), 'P', 'N') AND INLIST(ALLTRIM(m.kod_oper), '207', '209', '225', '227', '324', '314', '31G', '32G')
    ...
    DO CASE
      CASE INLIST(ALLTRIM(m.kod_oper), '207', '209', '324', '32G')
        ...
        DO CASE
          CASE ALLTRIM(m.val_card) == 'RUR'
            m.nls_cred = ALLTRIM(num_47423_no_rur_pos)

          CASE ALLTRIM(m.val_card) == 'USD'
            m.nls_cred = ALLTRIM(num_47423_no_usd_pos)

          CASE ALLTRIM(m.val_card) == 'EUR'
            m.nls_cred = ALLTRIM(num_47423_no_eur_pos)
...
* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ *
      ...
      CASE INLIST(ALLTRIM(m.kod_oper), '225')
        ...
        DO CASE
          CASE ALLTRIM(m.val_card) == 'RUR'
            m.nls_deb = ALLTRIM(num_47423_no_rur_mag)

          CASE ALLTRIM(m.val_card) == 'USD'
            m.nls_deb = ALLTRIM(num_47423_no_usd_mag)

          CASE ALLTRIM(m.val_card) == 'EUR'
            m.nls_deb = ALLTRIM(num_47423_no_eur_mag)
...
* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ *
      ...
      CASE INLIST(ALLTRIM(m.kod_oper), '227', '229', '314', '31G')
        ...
        DO CASE
          CASE ALLTRIM(m.val_card) == 'RUR'
            m.nls_deb = ALLTRIM(num_47423_no_rur_pos)

          CASE ALLTRIM(m.val_card) == 'USD'
            m.nls_deb = ALLTRIM(num_47423_no_usd_pos)

          CASE ALLTRIM(m.val_card) == 'EUR'
            m.nls_deb = ALLTRIM(num_47423_no_eur_pos)
...
* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ *
  ...
  CASE INLIST(ALLTRIM(exp_term.term), 'A', 'N') AND INLIST(ALLTRIM(m.kod_oper), '205')
    ...
    DO CASE
      CASE ALLTRIM(m.val_card) == 'RUR'
        m.nls_cred = ALLTRIM(num_47423_no_rur_mag)

      CASE ALLTRIM(m.val_card) == 'USD'
        m.nls_cred = ALLTRIM(num_47423_no_usd_mag)

      CASE ALLTRIM(m.val_card) == 'EUR'
        m.nls_cred = ALLTRIM(num_47423_no_eur_mag)
...
**************************************************** PROCEDURE start_47423_exp_term_no *****************************************************************
...
********************************************************** PROCEDURE start_47423_loc_term ***************************************************************
  ...
  CASE INLIST(ALLTRIM(loc_term.term), 'A') AND INLIST(ALLTRIM(m.kod_oper), '207', '209', '225', '227', '324', '314', '31G', '32G')
    ...
    DO CASE
      CASE INLIST(ALLTRIM(m.kod_oper), '207', '209', '324', '32G')
        ...
        DO CASE
          CASE ALLTRIM(m.val_card) == 'RUR'
            m.nls_cred = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_rur_atm))

          CASE ALLTRIM(m.val_card) == 'USD'
            m.nls_cred = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_usd_atm))

          CASE ALLTRIM(m.val_card) == 'EUR'
            m.nls_cred = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_eur_atm))
...
* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ *
      ...
      CASE INLIST(ALLTRIM(m.kod_oper), '225')
        ...
        DO CASE
          CASE ALLTRIM(m.val_card) == 'RUR'
            m.nls_deb = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_rur_mag))

          CASE ALLTRIM(m.val_card) == 'USD'
            m.nls_deb = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_usd_mag))

          CASE ALLTRIM(m.val_card) == 'EUR'
            m.nls_deb = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_eur_mag))
...
* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ *
      ...
      CASE INLIST(ALLTRIM(m.kod_oper), '227', '229', '314', '31G')
        ...
        DO CASE
          CASE ALLTRIM(m.val_card) == 'RUR'
            m.nls_deb = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_rur_atm))

          CASE ALLTRIM(m.val_card) == 'USD'
            m.nls_deb = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_usd_atm))

          CASE ALLTRIM(m.val_card) == 'EUR'
            m.nls_deb = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_eur_atm))
  ...
  CASE INLIST(ALLTRIM(loc_term.term), 'P', 'N') AND INLIST(ALLTRIM(m.kod_oper), '207', '209', '225', '227', '324', '314', '31G', '32G')
    ...
    DO CASE
      CASE INLIST(ALLTRIM(m.kod_oper), '207', '209', '324', '32G')
        ...
        DO CASE
          CASE ALLTRIM(m.val_card) == 'RUR'
            m.nls_cred = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_rur_pos))

          CASE ALLTRIM(m.val_card) == 'USD'
            m.nls_cred = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_usd_pos))

          CASE ALLTRIM(m.val_card) == 'EUR'
            m.nls_cred = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_eur_pos))
...
* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ *
      ...
      CASE INLIST(ALLTRIM(m.kod_oper), '227', '229', '314', '31G')
        ...
        DO CASE
          CASE ALLTRIM(m.val_card) == 'RUR'
            m.nls_deb = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_rur_pos))

          CASE ALLTRIM(m.val_card) == 'USD'
            m.nls_deb = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_usd_pos))

          CASE ALLTRIM(m.val_card) == 'EUR'
            m.nls_deb = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_eur_pos))
...
* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ *
  ...
  CASE INLIST(ALLTRIM(loc_term.term), 'A', 'N') AND INLIST(ALLTRIM(m.kod_oper), '205')
    ...
    DO CASE
      CASE ALLTRIM(m.val_card) == 'RUR'
        m.nls_cred = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_rur_mag))

      CASE ALLTRIM(m.val_card) == 'USD'
        m.nls_cred = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_usd_mag))

      CASE ALLTRIM(m.val_card) == 'EUR'
        m.nls_cred = IIF(INLIST(ALLTRIM(m.branch), '02', '06', '07'), ALLTRIM(poisk_numliz_term), ALLTRIM(num_47423_yes_eur_mag))
