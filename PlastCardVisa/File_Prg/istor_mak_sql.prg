************************************************************************************
*** Формирование истории остатков по карточному счету клиента из ПО "CARD-ВИЗА" ***
************************************************************************************
...
* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- *
      ...
      IF NOT LASTKEY() = 27  && Если не было выхода по клавише ESC
        ...
        DO poisk_client IN Istor_mak_sql
...
* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- *
        ...
        select_data = SQLEXEC(sql_num,"EXECUTE select_istor_mak_cln ?sel_card_acct","istor_mak")
...
* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- *
        ...
        SELECT DISTINCT TTOD(date_ost_m) AS data ;
          FROM istor_mak ;
          INTO CURSOR tov ;
          ORDER BY 1 DESC
        ...
        IF _TALLY <> 0
          ...
          IF NOT LASTKEY() = 27  && Если не было выхода по клавише ESC
            ...
            DO vibor_data IN Istor_mak_sql
    ...
    DO CASE
      CASE BAR() = 1  && Табличный просмотр данных
        ...
        SELECT branch, ref_client, card_nls, card_acct, val_card, fio_rus, name_bank_P AS name_p,;
          date_ost_m AS date_ostat, vxd_ost_m, debit_m, credit_m, isx_ost_m ;
          FROM istor_mak ;
          WHERE ALLTRIM(card_acct) == ALLTRIM(sel_card_acct) AND BETWEEN(DTOS(TTOD(date_ost_m)), DTOS(dt1), DTOS(dt2)) ;
          INTO CURSOR sel_ostatok ;
          ORDER BY branch, val_card, card_nls, date_ost_m
...
* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- *
      ...
      CASE BAR() = 3  && Печатный отчет на принтер
        ...
        SELECT branch, ref_client, card_nls, card_acct, val_card, fio_rus, name_bank_P AS name_p,;
          ...
          WHERE ALLTRIM(card_acct) == ALLTRIM(sel_card_acct) AND BETWEEN(DTOS(TTOD(date_ost_m)), DTOS(dt1), DTOS(dt2)) ;
          INTO CURSOR sel_ostatok ;
          ORDER BY branch, val_card, sort_nls, date_ost_m
          ...
          IF NOT LASTKEY() = 27  && Если не было выхода по клавише ESC
            ...
            SCAN
              ...
              SELECT branch, ref_client, card_nls, card_acct, val_card, fio_rus,;
                date_ostat, vxd_ost_m, debit_m, credit_m, isx_ost_m,;
                name_p, sort_nls ;
                FROM sel_ostatok ;
              ...
              IF _TALLY <> 0
                ...
                DO CASE
                  CASE LEN(ALLTRIM(prn_ostatok.card_acct)) < 20
                    ...
                    ima_frx = (put_frx) + ('istr_ostatok_3.frx')

                  CASE LEN(ALLTRIM(prn_ostatok.card_acct)) = 20

                    ima_frx = (put_frx) + ('istr_ostatok_4.frx')
...
BROWSE FIELDS ;
  ...
  vxd_ost_m:H = 'Вход. остаток.':P = '999 999 999.99',;
  debit_m:H = 'Дебет':P = '999 999 999.99',;
  credit_m:H = 'Кредит':P = '999 999 999.99',;
  isx_ost_m:H = 'Исх. остаток':P = '999 999 999.99',;
